...
def create_table(conn):
    ...
    create_table_sql = """ CREATE TABLE IF NOT EXISTS measurements (
                                ...
                                freq FLOAT ,
                                puissance_centrale_tri FLOAT ,
                                energie_centrale_tri FLOAT ,
                                courant_centrale_tri FLOAT ,
                                tens_ph_n_centrale_tri FLOAT
                            ); """
...
def add_measurement(conn, time, freq, puissance_centrale_tri, energie_centrale_tri, courant_centrale_tri, tens_ph_n_centrale_tri):
    """
    ...
    """
    cur = conn.cursor()
    measurements_set = ''' INSERT INTO measurements(time, freq, puissance_centrale_tri, energie_centrale_tri, courant_centrale_tri, tens_ph_n_centrale_tri) VALUES(?,?,?,?,?,?)'''
    cur.execute(measurements_set, (time, freq, puissance_centrale_tri, energie_centrale_tri, courant_centrale_tri, tens_ph_n_centrale_tri))
    conn.commit()
...
def export_to_csv(conn_db):
  # Export data into CSV file
    ...
    with open("data_centrale_tri.csv", "w") as csv_file:
        csv_writer = csv.writer(csv_file, delimiter="\t")
        csv_writer.writerow([i[0] for i in cursor.description])
        csv_writer.writerows(cursor)

    dirpath = os.getcwd() + "/data_centrale_tri.csv"
